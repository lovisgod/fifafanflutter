...
LoadingDialogType _LoadingDialogType;
...
class LoadingDialog {
  ...
  LoadingDialog(BuildContext context,
      ...
      Widget customBody}) {
    _context = context;
    _LoadingDialogType = type ?? LoadingDialogType.Normal;
  ...
  void style(
    ...
    if (_LoadingDialogType == LoadingDialogType.Download) {
  ...
  Future<bool> hide() async {
    try {
      if (_isShowing) {
        ...
        if (_showLogs) debugPrint('LoadingDialog dismissed');
        return Future.value(true);
      } else {
        if (_showLogs) debugPrint('LoadingDialog already dismissed');
  ...
  Future<bool> show() async {
    try {
      if (!_isShowing) {
        ...
        if (_showLogs) debugPrint('LoadingDialog shown');
        _isShowing = true;
        return true;
      } else {
        if (_showLogs) debugPrint("LoadingDialog already shown/showing");
        return false;
      }
    } catch (err) {
      _isShowing = false;
...
class _BodyState extends State<_Body> {
  ...
  void dispose() {
    _isShowing = false;
    if (_showLogs) debugPrint('LoadingDialog dismissed by back button');
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final text = Expanded(
      child: _LoadingDialogType == LoadingDialogType.Normal
          ? Text(
              _dialogMessage,
